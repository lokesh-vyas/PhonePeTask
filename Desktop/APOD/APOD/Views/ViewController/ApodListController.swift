...
extension ApodListController: UITableViewDataSource, UITableViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if currentAPOD.imageURL == nil {
            
            if currentAPOD.mediaType == "image" {
                
                if let url = URL(string: currentAPOD.imageURL) {
                ...
                if currentAPOD.imageURL.contains("youtube"){
                    
                    let thumbnailURL = "https://img.youtube.com/vi/" + (currentAPOD.imageURL.youtubeID ?? "k_2yFEzwNG4") + "/mqdefault.jpg" // the default is a mock video
